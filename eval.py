...
def evaluate(partition,island_type,model,split_f,task,k_crosses=5,score_metrics=[],
    ...
    estimators={island:model(n_jobs=-1) for island in partition.keys()}
...
def run(partitions_path,results_path,islands_to_test,splits_to_use,model_type,k_crosses,meta_fit_on_val,random_state,verbose_level):
    ...
    for ith_file,p_file in enumerate(fnames):
        ...
        # if 'Hotel Reservations_dirY_alpha=0.5_clients=10_2.pkl' not in p_file:continue
...
if __name__=='__main__':
    ...
    islands_to_test={
        ...
        # 'RandomBinaryNIsland prop=1.0':(islands.RandomBinaryNIsland,{'prop':1.0}), # 1

        # 'Random':(islands.RandomContIsland,{}), #2
        # 'RandomBinaryNIsland prop=0.5':(islands.RandomBinaryNIsland,{'prop':0.5}), 
        # 'Linear C=0.00001':(islands.StackingLinearIsland,{'C':0.00001}),

        # 'Linear C=0.001':(islands.StackingLinearIsland,{'C':0.001}), #3
        # 'Linear C=0.1':(islands.StackingLinearIsland,{'C':0.1}),
        # 'Linear C=1':(islands.StackingLinearIsland,{'C':1}),

        # 'Linear C=10':(islands.StackingLinearIsland,{'C':10}), #4
        # 'Linear C=100':(islands.StackingLinearIsland,{'C':100}),
        # 'Linear prop=1.0':(islands.StackingLinearIsland,{'prop':1.0}), 

        # 'Linear prop=0.75':(islands.StackingLinearIsland,{'prop':0.7}), #5
        # 'Linear prop=0.50':(islands.StackingLinearIsland,{'prop':0.5}),
        # 'Linear prop=0.25':(islands.StackingLinearIsland,{'prop':0.25}),

        # 'Forest prop=1.0':(islands.StackingForestIsland,{'prop':1.0}), #7
        # 'Forest prop=0.75':(islands.StackingForestIsland,{'prop':0.75}),

        'Forest prop=0.50':(islands.StackingForestIsland,{'prop':0.50}), #8
        'Forest prop=0.25':(islands.StackingForestIsland,{'prop':0.25}),
    ...
    # debug:
    # rm -r tmp/tmp ; python3 -u eval.py --partitions_dir partitions/model_ultra_quick --output_dir tmp/tmp --local_splits -v 2 --k_crosses 1
    ...
    # nohup python3 -u eval.py --partitions_dir partitions/model_experiments --output_dir results/model_experiments --global_splits --local_splits --verbose_level 2 --k_crosses 5 &
